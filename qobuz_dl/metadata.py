...
# Suporta múltiplos formatos de capa
EMB_COVER_NAMES = [
    "embed_cover.jpg",
    "embed_cover.jpeg",
    "embed_cover.png",
...
def tag_flac(
    ...
    if not settings.no_track_number_tag:
        tags["TRACKNUMBER"] = str(
            qobuz_item.get("track_number", "1")
        )

    if not settings.no_track_total_tag:
        tags["TRACKTOTAL"] = str(
            qobuz_album.get("tracks_count", "1")
        )

    if not settings.no_disc_number_tag:
        tags["DISCNUMBER"] = str(
            qobuz_item.get("media_number", "1")
        )

    if not settings.no_disc_total_tag:
        tags["DISCTOTAL"] = str(
            qobuz_album.get("media_count", "1")
        )
...
def _get_tags_to_add(
    ...
    if not settings.no_label_tag:
        tags["LABEL"] = re.sub(
            r'\s+',
            ' ',
            qobuz_album.get("label", {}).get("name", "")
        )
    ...
    if not settings.no_media_type_tag:
        tags["MEDIATYPE"] = qobuz_album.get(
            "product_type",
            ""
        ).upper()
    ...
    if not settings.no_explicit_tag:
        tags["ITUNESADVISORY"] = (
            "1"
            if qobuz_album.get("parental_warning", False)
            else "0"
        )
    ...
    if audio_info:
        ...
        rg_gain = audio_info.get(
            "replaygain_track_gain"
        )

        rg_peak = audio_info.get(
    ...
    if performers_str:
        ...
        for performer_block in performers_str.split(" - "):
            ...
            parts = [
                p.strip()
                for p in performer_block.split(", ")
            ]
