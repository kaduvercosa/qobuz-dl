...
class Download:
    ...
    async def download_release(self):
        ...
        if delay_time > 0:
            await safe_print_async(f"{YELLOW}[*] Safety Delay active: Multithreading disabled (Sequential mode).{OFF}")
            active_workers = 1
        elif active_workers > 1:
            is_parallel = True
            await safe_print_async(f"{YELLOW}[*] Multithreading Enabled ({active_workers} workers): UI optimized for clean parallel logging.{OFF}")
        ...
        try:
            ...
            if "goodies" in album_meta:
                await _download_goodies(album_meta, dirn)
                
            if getattr(self, 'booklet_only', False):
                await safe_print_async(f"{YELLOW}[*] --booklet-only flag active. Skipping audio tracks.{OFF}")
            ...
            for i in album_meta["tracks"]["items"]:
                ...
                try:
                    ...
                    await safe_print_async(f"{RED}[!] API Error for track {i.get('track_number', 'unknown')} (ID: {i['id']}): {e}{OFF}")
                    await safe_print_async(f"{YELLOW}[*] Skipping track and continuing with the album...{OFF}")
                    count += 1
                    failed_tracks += 1
                    continue
                ...
                if "sample" not in parse and parse["sampling_rate"]:
                    ...
                    multiple = i.get("media_number") if is_multiple else None
                    tasks.append(bound_download(dirn, count, parse, i, album_meta, is_mp3, multiple, is_parallel))
                else:
        ...
        if is_standard_album and working_dirn == inprogress_dirn:
            ...
            if aborted_by_user:
                await safe_print_async(f"{YELLOW}[!] Download aborted. Folder successfully marked as [INCOMPLETE].{OFF}")
    ...
    async def download_track(self):
        ...
        if "sample" not in parse and parse["sampling_rate"]:
            ...
            await self._determine_formats(album_meta=track_meta.get("album", {}), album_attr=None, tracks_meta=[track_meta],
    ...
    async def _download_and_tag(
        ...
        if os.path.exists(final_file):
            await safe_print_async(f"{CYAN}[*] Skipping: {os.path.basename(final_file)} (Already exists){OFF}")
            return True
        ...
        for attempt_fmt in qualities_to_try:
            ...
            if attempt_fmt != int(self.quality):
                await safe_print_async(f"{YELLOW}[!] Automatic downgrade: Attempting to save in {TIER_NAMES[attempt_fmt]}...{OFF}")
            ...
            try:
                ...
                if "url" in fresh_track_dict:
                    try:
                        await tqdm_download(fresh_track_dict["url"], filename, desc, is_parallel=is_parallel)
                        success = True
                        final_fmt = attempt_fmt
                        break
                    except Exception as e:
                        if abort_event.is_set(): return False
                        await safe_print_async(f"{YELLOW}[!] Akamai block detected. Activating fallback segmented download...{OFF}")
                        fresh_track_dict = await get_fresh_url(force_segments=True)
                
                if "url_template" in fresh_track_dict:
                    await tqdm_download_segments(fresh_track_dict, filename, desc, is_parallel=is_parallel)
                    success = True
                    final_fmt = attempt_fmt
                    break
                elif not success:
    ...
    async def _determine_formats(self, album_meta, album_attr, tracks_meta, track_attr, is_track, file_format, settings: QobuzDLSettings):
        format_combinations = [
    ...
    async def _generate_tracklist(self, meta, dirn, album_title, file_format, bit_depth, sampling_rate):
        import re
        import textwrap
        ...
        try:
            ...
            await safe_print_async(f"{GREEN}  L Completed: Digital Booklet.txt (Credits & Review){OFF}")
        except Exception as e:
            await safe_print_async(f"{RED}[!] Error creating booklet: {e}{OFF}")

    async def _append_lyrics_to_booklet(self, dirn, album_title):
        import re
        ...
        if lyrics_to_append:
            try:
                ...
                await safe_print_async(f"{CYAN}[+] Lyrics cleanly formatted and appended to Digital Booklet.{OFF}")
            except Exception as e:
...
async def _get_extra(item, dirn, extra="cover.jpg", art_size=None, og_quality=False):
    if abort_event.is_set(): return
...
async def tqdm_download_segments(track_url_dict, fname, track_name, is_parallel=False):
    if abort_event.is_set(): return
    ...
    async def get_seg_size(session, seg_num):
    ...
    async with aiohttp.ClientSession() as session:
        tasks = [get_seg_size(session, i) for i in range(n_segments + 1)]
    ...
    async def fetch_segment_fluid(session, seg_num, bar):
...
async def _download_goodies(album_meta, dirn):
    if abort_event.is_set(): return
    try:
        for goody in album_meta.get("goodies", []):
            ...
            await _get_extra(goody.get("url"), dirn, extra=goody_name)
    except Exception as e:
